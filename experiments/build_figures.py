"""
Genera figuras para el informe a partir de los snapshots en results/.
Salida: informe/figs/*.pdf (también .png para previsualización)
"""
import json
...
# 1. Hit rate: distribución + política
def fig1():
    ...
    for b,h in zip(bars,hrs):
        ax.text(b.get_x()+b.get_width()/2, h+0.008, f"{h:.3f}", ha="center",
    ...
    ax.grid(axis="y",alpha=0.3,zorder=0); fig.tight_layout()
    save(fig,"fig1_hit_rate_distribution")


# 2. Zipf vs Uniforme por política
def fig2():
    fig, ax = plt.subplots(figsize=(8,4.5))
    x = np.arange(len(POLICIES)); w = 0.35
    for i, dist in enumerate(DISTS):
        hrs = [v(snap(f"{pol}_50mb_{dist}"),"hit_rate") for pol in POLICIES]
        bars = ax.bar(x+(i-0.5)*w, hrs, w, label=dist.capitalize(),
                      color=C[dist], alpha=0.9, zorder=2)
        for b,h in zip(bars,hrs):
    ...
    fig.tight_layout(); save(fig,"fig2_zipf_vs_uniform")
...
def fig3():
    ...
    ax.set_xlabel("Tamaño del caché"); ax.set_ylabel("Hit rate")
    ax.set_ylim(0.7,0.9)
    ax.set_title("Hit rate vs. tamaño de caché (Zipf s=1.5)",pad=10)
    ax.legend(title="Política"); ax.grid(axis="y",alpha=0.3,zorder=0)
    ax.text(0.5,0.04,"Sin evicciones en ningún tamaño — el keyspace cabe en 50 MB",
            ha="center", va="bottom", transform=ax.transAxes,
            fontsize=9, color="grey", style="italic")
    fig.tight_layout(); save(fig,"fig3_size_effect")
...
def fig4():
    ...
    for i, dist in enumerate(DISTS):
        ...
        bars = ax.bar(x+(i-0.5)*w, thrs, w, label=dist.capitalize(),
                      color=C[dist], alpha=0.9, zorder=2)
        for b,t in zip(bars,thrs):
            ax.text(b.get_x()+b.get_width()/2, t+0.4, f"{t:.1f}",
                    ha="center", fontsize=9)
    ...
    fig.tight_layout(); save(fig,"fig4_throughput")


# 5. Latencia hit vs miss
def fig5():
    fig, ax = plt.subplots(figsize=(9,4.5))
    x = np.arange(len(POLICIES)); w = 0.2
    ...
    fig.tight_layout(); save(fig,"fig5_latency_hit_miss")


# 6. Desglose por consulta Q1-Q5
def fig6():
    ...
    for i, (label, name, color) in enumerate([
        ...
        for b,y in zip(bars,ys):
            ax.text(b.get_x()+b.get_width()/2, y+0.01, f"{y:.2f}",
                    ha="center", fontsize=9)
    ax.set_xticks(x); ax.set_xticklabels(queries)
    ax.set_ylabel("Hit rate"); ax.set_ylim(0,1.05)
    ax.set_title("Hit rate por tipo de consulta Q1–Q5 (caché 50 MB, LRU)",pad=10)
    ax.legend(); ax.grid(axis="y",alpha=0.3,zorder=0)
    fig.tight_layout(); save(fig,"fig6_by_query")


# 7. Cache efficiency
def fig7():
    labels = ["LRU_50mb_zipf","LFU_50mb_zipf","FIFO_50mb_zipf","LRU_50mb_zipf_long"]
    pretty = ["LRU\n(25 s)","LFU\n(25 s)","FIFO\n(25 s)","LRU\n(180 s)"]
    ...
    for b,e in zip(bars,effs):
        ax.text(b.get_x()+b.get_width()/2, e+3, f"{e:.0f}",
