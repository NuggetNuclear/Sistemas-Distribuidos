...
\begin{figure}[h]
\centering\includegraphics[width=0.8\textwidth]{figs/fig1_hit_rate_distribution.pdf}
...
\begin{figure}[h]
\centering\includegraphics[width=0.85\textwidth]{figs/fig4_throughput.pdf}
...
\begin{figure}[h]
\centering\includegraphics[width=0.88\textwidth]{figs/fig2_zipf_vs_uniform.pdf}
...
\begin{figure}[h]
\centering\includegraphics[width=\textwidth]{figs/fig3_size_effect.pdf}
...
\subsubsection*{Corrida de saturación deliberada (LRU, 1.2\,MB, Uniforme, 60\,s)}

Para evidenciar empíricamente el comportamiento de la caché bajo presión
real de memoria se añadió una \emph{única corrida corta} configurada para
forzar evicciones. La justificación de cada parámetro es la siguiente:

\begin{itemize}[leftmargin=*,itemsep=2pt]
\item \textbf{Tamaño $\mathbf{1.2}$\,MB.} Redis 7 reserva $\approx 950$\,KB
  como \texttt{used\_memory\_overhead} (estructuras internas, slot tables,
  conexiones), independientemente del número de keys. Cualquier
  \texttt{maxmemory} por debajo de ese piso provoca que toda inserción
  falle silenciosamente sin que el cliente lo perciba (verificado:
  \texttt{maxmemory}\,=\,256\,KB produjo 0 keys residentes y 0 hits). Con
  \texttt{maxmemory}\,=\,1.2\,MB el presupuesto efectivo para datos es
  $\sim$250\,KB, suficiente para alojar entre 60 y 150 entradas a la vez,
  pero \emph{insuficiente} para el keyspace alcanzable.
\item \textbf{Distribución Uniforme.} Maximiza la diversidad de keys
  consultadas (las cinco zonas, cinco tipos de consulta y 21 niveles de
  \texttt{confidence\_min} aparecen con igual probabilidad), llevando al
  sistema rápidamente a generar más keys distintas que las que caben en
  caché.
\item \textbf{Duración 60\,s a 60\,qps.} Produce $\sim$3\,600 invocaciones
  al traffic generator; con la cota cuadrática del keyspace
  ($5 \times 5 \times 21 = 525$ + pares Q4) y la latencia simulada que
  satura el response generator, las consultas efectivamente cursadas
  exceden largamente las 74 keys que el caché puede sostener
  simultáneamente, garantizando rotación continua.
\end{itemize}
...
Hit rate                        & 0.1903 \\
Miss rate                       & 0.8097 \\
Throughput (qps)                & 10.02 \\
Latencia p50 hit / miss (ms)    & 0.28 / 1\,680 \\
Latencia p95 hit / miss (ms)    & 0.49 / 2\,503 \\
Cache efficiency (ms/consulta)  & 334.3 \\
Keys residentes                 & 74 \\
\texttt{used\_memory} / \texttt{maxmemory} & 1.16\,MB / 1.20\,MB (97\,\%) \\
\textbf{Evicciones totales}     & \textbf{475} \\
\textbf{Eviction rate (/min)}   & \textbf{408.4} \\
\bottomrule
\end{tabular}
\caption{Resultados de la corrida de saturación deliberada
(LRU, 1.2\,MB, Uniforme, 60\,s).}\label{tab:eviction}
\end{table}

\textbf{Lectura del Cuadro~\ref{tab:eviction}.} El caché se mantiene al
97\,\% de \texttt{maxmemory} con 74 keys residentes, mientras LRU expulsa
475 entradas durante la ventana (rate 408/min). El hit rate cae a 19\,\%,
muy por debajo del 27\,\% Uniforme con 50\,MB, exactamente porque las
keys recientemente insertadas son expulsadas antes de poder ser reutilizadas;
es la \emph{firma operacional} de un caché bajo presión, no un defecto
de la implementación. La caída del hit rate por consulta es proporcional al
tamaño del keyspace de cada una: Q5 (sólo 9 valores de \texttt{bins} sobre
5 zonas $=$ 45 keys) cae a 40\,\%; Q4 (cuadrático, $\geq 525$ keys) cae a
8\,\%. Este experimento valida que la lógica de evicción funciona
correctamente; en el régimen oficial (50/200/500\,MB con 319 mil
edificaciones) el keyspace simplemente no alcanza a saturar el caché y por
eso se observan cero evicciones.

\subsection{Latencia hit vs.\ miss y cache efficiency}

\begin{figure}[h]
\centering\includegraphics[width=0.9\textwidth]{figs/fig5_latency_hit_miss.pdf}
...
\begin{figure}[h]
\centering\includegraphics[width=0.75\textwidth]{figs/fig7_cache_efficiency.pdf}
...
\begin{figure}[h]
\centering\includegraphics[width=0.85\textwidth]{figs/fig6_by_query.pdf}
...
LRU  Uniforme 1.2\,MB 60\,s & 0.1903 & 10.02 & 0.28 / 1\,680 & 0.49 / 2\,503 & 408.4 & \phantom{1\,}334.3 \\
\bottomrule
\end{tabular}
\caption{Las cinco métricas del enunciado para cada configuración
(\textit{Hit rate}, \textit{Throughput}, \textit{Latencia p50/p95},
\textit{Eviction rate}, \textit{Cache efficiency}). La última fila
corresponde a la corrida de saturación deliberada (Cuadro~\ref{tab:eviction})
y es la única que registra evicciones por la justificación discutida en
la Sec.~4.3.}\label{tab:metrics}
\end{table}
...
\item \textbf{Los tres tamaños de caché (50/200/500\,MB) son equivalentes}
  ...
  es ruido estadístico (\textless 1\,pp). Una corrida adicional de
  saturación deliberada (LRU 1.2\,MB Uniforme 60\,s) sí produjo
  475 evicciones (rate 408/min) y un hit rate de 19\,\%, validando la
  lógica de evicción del sistema (Cuadro~\ref{tab:eviction}).
